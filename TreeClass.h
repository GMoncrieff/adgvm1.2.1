...
class clTree
{
	...
	public:
		...
		void RunPhysiology( double A0, double RmL, double mmsTOkgd, double T, double wind,
							double *G_theta_weighted, double G_theta_3, double Ca, double P, double rh,
							double comp_height, double C4_grass_height, double C3_grass_height, int day,
							double T_fac, int frost, int comp_type, double C34_ratio, double *thickness );
		...
		double getBld()				{ double tmp=Bld_; Bld_=0; return tmp; } // get dead biomass and set to pool to zero
		double getBsd()				{ double tmp=Bsd_; Bsd_=0; return tmp; } // get dead biomass and set to pool to zero
		double getBrd()				{ double tmp=Brd_; Brd_=0; return tmp; } // get dead biomass and set to pool to zero
		double getLeafCombustion()	{ return leaf_combustion_; }
		double getStemCombustion()	{ return stem_combustion_; }
...
#		ifdef S_ELEPHANTS
		...
		void   setBs(  double bm )	{ Bs_  = bm; return; }
		void   setBld( double bm )	{ Bld_ = bm; return; }
#		endif
};
...
int clTree::WillIDieAfterFire()
{
	...
	if ( drand48() > RESPROUTING_PROB[tree_type_] ) will_I = 1;
...
double clTree::TopKillProb( double intensity )
{
	...
	if ( height_>0 )
		prob =
		(    exp( TOP_KILL_CONST[tree_type_] - TOP_KILL_H[tree_type_]*log( height_ ) + TOP_KILL_I[tree_type_]*sqrt( intensity ) ) )/
