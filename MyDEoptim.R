...
'MyDEoptim' <- function(FUN, lower, upper, initial,control = list(), ...) {
...
########Steve-start######################################
  ni<-nrow(initial)
  pop[1:ni,] <- initial #steves code to put a user definited individual in population
########Steve-end########################################
  ...
  while (iter <= con$itermax & bestval > con$VTR){
    ...
    if (con$refresh > 0 & iter %% con$refresh == 0)
      cat("iteration: ", iter,
          #Steve "best member: " , round(bestmem, con$digits),
...
'plot.DEoptim' <- function(x, plot.type = c("bestmemit","bestvalit"), ...){
  ...
  if (plot.type == "bestmemit"){
    ...
    plot(1:n, z$bestvalit, type="b", pch=".",
