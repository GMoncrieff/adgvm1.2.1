#ifndef GrassPopClass_h___
#define GrassPopClass_h___
...
class clGrassPop
{
	private:
		...
		clGrass	*Grasses;					// array of plants
	...
	public:
		...
		double getLeafBmLive()				{ return leaf_bm_live_; }			// kg/m^2
		double getStemBmLive()				{ return stem_bm_live_; }			// kg/m^2
		double getRootBmLive()				{ return root_bm_live_; }			// kg/m^2
		
		double getLeafBmDeadSt()			{ return leaf_bm_dead_st_; }		// kg/m^2
		double getLeafBmDeadLy()			{ return leaf_bm_dead_ly_; }		// kg/m^2
		double getStemBmDeadSt()			{ return stem_bm_dead_st_; }		// kg/m^2
		double getStemBmDeadLy()			{ return stem_bm_dead_ly_; }		// kg/m^2
		double getRootBmDead()				{ return root_bm_dead_; }			// kg/m^2
		double getLeafLiveCombustion()		{ return leaf_live_comb_; }			// kg/m^2
		double getLeafDeadStCombustion()	{ return leaf_dead_st_comb_; }		// kg/m^2
		double getLeafDeadLyCombustion()	{ return leaf_dead_ly_comb_; }		// kg/m^2
		double getGPP()						{ return gpp_; }					// kg/m^2
		double getRma()						{ return Rma_; }					// kg/m^2
		double getRgr()						{ return Rgr_; }					// kg/m^2
		double getSoilNWL()					{ double tmp=soil_nwl_; soil_nwl_=0.; return tmp; }	// in kg/m^2
		double getC34Ratio()				{ return C34_ratio_; }
		double getHeight( int i )			{ return Grasses[i].getHeight(); }
...
#       ifdef S_ELEPHANTS
		double setStateAfterElephants( double biomass_consumption, double p_canopy );
		double getLeafBiomassOpn()					{ return Grasses[1].getBl(); }
		double getLeafBiomassCan()					{ return Grasses[0].getBl(); }
		double getDeadStandingLeafBiomassOpn()		{ return Grasses[1].getBld(); }
		double getDeadStandingLeafBiomassCan()		{ return Grasses[0].getBld(); }
#       endif
};
...
void clGrassPop::addGrass( double init_mass, int grass_type )
{
	...
	if ( pop_size_>0 )
		Grasses = (clGrass *) realloc(Grasses, (pop_size_+1)*sizeof(clGrass));
	else if ( pop_size_==0 )
		Grasses = (clGrass *) malloc((pop_size_+1)*sizeof(clGrass));
	
	Grasses[pop_size_] = Grass;
	pop_size_++;
...
double clGrassPop::getEt( double T, double P, double radnet_day, double s12, double SP_HEAT,
						  double gama12, double rho12, double VPD12, double p_canopy )
{
	double Et_grasses = 0;
...
#ifdef S_ELEPHANTS
double clGrassPop::setStateAfterElephants( double biomass_consumption, double p_canopy )
{
    double tmp_available;
    double tmp_remove;
    double rem_fraction;
    
//     cout << biomass_consumption ;
    
    if ( biomass_consumption > 0.01 )
    {
        tmp_available = (1.-p_canopy)*getLeafBiomassOpn()*10000.;
        rem_fraction  = MyMin( 0.9, biomass_consumption/tmp_available );
        tmp_remove = (10000.*(1.-p_canopy)*getLeafBiomassOpn()*rem_fraction);
        Grasses[1].setBl( getLeafBiomassOpn()*(1.-rem_fraction) );
        biomass_consumption -= tmp_remove;
        tmp_available = (1.-p_canopy)*getLeafBiomassOpn()*10000.;
    }
    
    if ( biomass_consumption > 0.01 )
    {
        tmp_available = p_canopy*getLeafBiomassCan()*10000.;
        rem_fraction  = MyMin( 0.9, biomass_consumption/tmp_available );
        tmp_remove = (10000.*p_canopy*getLeafBiomassCan()*rem_fraction);
        Grasses[0].setBl( getLeafBiomassCan()*(1.-rem_fraction) );
        biomass_consumption -= tmp_remove;
        tmp_available = p_canopy*getLeafBiomassCan()*10000.;
    }
    ...
    if ( biomass_consumption > 0.01 )
    {
        tmp_available = (1.-p_canopy)*getDeadStandingLeafBiomassOpn()*10000.;
        rem_fraction  = MyMin( 0.9, biomass_consumption/tmp_available );
        tmp_remove = (10000.*(1.-p_canopy)*getDeadStandingLeafBiomassOpn()*rem_fraction);
        Grasses[1].setBld( getDeadStandingLeafBiomassOpn()*(1.-rem_fraction) );
        biomass_consumption -= tmp_remove;
        tmp_available = (1.-p_canopy)*getDeadStandingLeafBiomassOpn()*10000.;
    }
    ...
    if ( biomass_consumption > 0.01 )
    {
        tmp_available = p_canopy*getDeadStandingLeafBiomassCan()*10000.;
        rem_fraction  = MyMin( 0.9, biomass_consumption/tmp_available );
        tmp_remove = (10000.*p_canopy*getDeadStandingLeafBiomassCan()*rem_fraction);
        Grasses[0].setBld( getDeadStandingLeafBiomassCan()*(1.-rem_fraction) );
        biomass_consumption -= tmp_remove;
        tmp_available = p_canopy*getDeadStandingLeafBiomassCan()*10000.;
    }
    
    if ( biomass_consumption<0.001 ) biomass_consumption=0;
    
//     cout << "grass left " << biomass_consumption << endl;
