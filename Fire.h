...
double N2Ograss(  double bm ) { return bm*0.44*0.0090*0.0076*1.57; }
double N2Oleaf(   double bm ) { return bm*0.49*0.0127*0.0076*1.57; }
double N2Ocoarse( double bm ) { return bm*0.5 *0.0074*0.0076*1.57; }
double N2Oheavy(  double bm ) { return bm*0.5 *0.0074*0.0076*1.57; }
double N2Oshrub(  double bm ) { return bm*0.5 *0.0093*0.0076*1.57; }

double CH4grass(  double bm ) { return bm*0.44*0.0035*1.33; }
double CH4leaf(   double bm ) { return bm*0.49*0.0035*1.33; }
double CH4coarse( double bm ) { return bm*0.5 *0.0035*1.33; }
double CH4heavy(  double bm ) { return bm*0.5 *0.0035*1.33; }
double CH4shrub(  double bm ) { return bm*0.5 *0.0035*1.33; }
				  ...
				  double wind_speed, int day, double hum, double tmp, double soil_moisture )
{
	...
	#ifdef S_MANIP_FIRE
	if ( drand48()<(IGNITION_PROB*glob_fire_param) )
	#else
	if ( drand48()<IGNITION_PROB )
	#endif
	{
...
void GetIgnitions( arryYear ignitions, double param )
{
	...
	{
		#ifdef S_MANIP_FIRE
		if ( drand48() < IGNITION_PAR_1*param+IGNITION_PAR_2*glob_fire_param  ) ignitions[i] = 1.;
		#else
		if ( drand48() < IGNITION_PAR_1*param+IGNITION_PAR_2 ) ignitions[i] = 1.;
		#endif
		else                                                   ignitions[i] = 0.;
	}
